...
| 설문 기반 일정 생성 | 18 | 3 | 설문, 장소 검색·선택, V1 동기 일정 생성·조회와 V2·V3 확장 계약 |
...
| 내 설문 제출·수정 | `PUT` | `/api/trips/{tripId}/survey` | 답변과 제외 카테고리를 원자적으로 교체 | 여행 멤버 본인 | `IMPLEMENTED` |
| 설문 현황·집계 조회 | `GET` | `/api/trips/{tripId}/survey-summary` | 제출률과 그룹 취향 집계 조회 | 여행 멤버 | `APPROVED` |
| V1 일정 동기 생성 | `POST` | `/api/trips/{tripId}/schedule` | 이미 추천된 장소 6개의 최단 동선을 계산하고 Day 1 일정 저장·반환 | 방장 | `APPROVED` |
| 생성 일정 조회 | `GET` | `/api/trips/{tripId}/schedule` | 현재 V1 일정의 방문 순서와 좌표 간 직선거리 조회 | 여행 멤버 | `APPROVED` |
...
설문을 제출한 활성 멤버는 다시 제출할 수 있다. 재제출은 기존 답변과 제외 카테고리 전체를 원자적으로 교체하고 최신 설문 집계에 즉시 반영한다. AI 생성 중에 재제출해도 이미 실행 중인 작업은 시작 시점의 `schedule_generation_snapshots`만 사용하므로 해당 작업의 생성 결과에는 영향을 주지 않는다. AI 생성 시작은 신규 멤버 참여를 종료하지만 기존 멤버의 재제출을 그 자체로 차단하지 않는다.

V1 동기 일정 생성 요청이 설문 입력을 수집한 뒤 재제출해도 이미 실행 중인 요청의 입력과 결과에는 영향을 주지 않는다.
...
일정 제외 항목은 장소가 아니라 사전 등록된 카테고리이며 여러 개를 선택할 수 있다. Spring 서버는 AI 일정 생성 시작 시 제출 완료된 활성 멤버들의 원본 5점 점수와 선택된 제외 카테고리 코드를 불변 스냅샷으로 저장한 뒤 FastAPI 서버에 전달한다. 클라이언트는 FastAPI를 직접 호출하지 않는다.

Spring 백엔드는 장소를 자체 추천하지 않는다. 장소 추천, 포함·제외 여부, 경로 구성과 제외 사유 생성은 FastAPI가 전담한다. Spring은 요청 데이터 검증, 스냅샷 저장, FastAPI 작업 요청, 결과 저장과 사용자 API 제공만 담당한다.
...
| 채팅방 입장 | `PUT` | `/api/regional-chat-rooms/{roomId}/members/me` | 정책 동의 확인 후 현재 사용자 입장 | Access Token | `APPROVED` |
| 채팅방 퇴장 | `DELETE` | `/api/regional-chat-rooms/{roomId}/members/me` | 현재 사용자의 채팅방 참여 종료 | 채팅방 참여자 | `APPROVED` |
| 메시지 이력 조회 | `GET` | `/api/regional-chat-rooms/{roomId}/messages` | 과거 이력은 cursor, 재연결 누락분은 `afterMessageId`로 20개씩 조회 | 채팅방 참여자 | `APPROVED` |
| 채팅 이미지 업로드 요청 | `POST` | `/api/regional-chat-rooms/{roomId}/image-upload-requests` | 멱등성 Key 없이 JPEG·PNG·WebP, 최대 5MB 업로드 준비 | 채팅방 참여자 | `APPROVED` |

### 6.1 실시간 인터페이스 후보

| 인터페이스 | 방식 | 경로 | 목적 | 상태 |
| --- | --- | --- | --- | --- |
| 지역 채팅 연결·메시지 | Native WebSocket 기반 Spring WebSocket + STOMP, Simple Broker | `/ws` | 방 구독, 텍스트·이미지 메시지 전송과 수신, 발신자 처리 결과와 제재 통지 | `APPROVED` |

HTTP는 방·정책·참여·과거 이력·이미지 업로드를 담당하고 실시간 인터페이스는 새 메시지와 제재 통지를 담당한다. STOMP `CONNECT`의 Bearer Token으로 인증하고 토큰 만료 시 갱신·재연결·재구독한 뒤 마지막 수신 `messageId`를 `afterMessageId`로 전달해 HTTP 이력 API에서 누락 메시지를 복구한다. 과거 방향 cursor는 30분 동안 유효하고 두 조회 기준은 동시에 사용할 수 없다. 전송은 `/app/regional-chat-rooms/{roomId}/messages`, 방 구독은 `/topic/regional-chat-rooms/{roomId}/messages`, 메시지 처리·정책·제재 개인 이벤트는 `/user/queue/chat-events`를 사용한다. 실시간 메시지는 UUIDv7 `clientMessageId`로 사용자별 중복 저장을 방지하고, 메시지 처리 결과는 발신 세션의 `CHAT_MESSAGE_RESULT`, 구독 성공은 요청 세션의 STOMP `RECEIPT`, 구독 실패는 JSON 오류 응답을 포함한 STOMP `ERROR`, 공개 메시지는 승인된 방 구독자의 `CHAT_MESSAGE_CREATED`로 전달한다. 이 이벤트와 프레임은 DB에 별도 저장하지 않는다. 최초 입장에서는 개인 이벤트 구독, 고유 `receipt` 헤더를 포함한 방 구독, 같은 값을 `receipt-id`로 가진 `RECEIPT` 확인, 최신 HTTP 이력 조회 순으로 처리하고 조회 중 수신한 실시간 메시지를 `messageId`로 중복 제거해 병합한다. Heartbeat는 클라이언트·서버 모두 10초 간격으로 사용하고 네트워크 단절 후 자동 재연결은 제공하지 않는다. 연결·구독과 STOMP 세션 상태는 Redis 없이 단일 애플리케이션 인스턴스 메모리에서 관리한다. 사용자별 다중 연결을 허용하고 개발 Origin은 `http://localhost:5173`만 허용한다. 로그아웃 시 RDB의 채팅방 가입 상태는 유지하고 현재 STOMP 연결·구독만 종료한다. 채팅 기능 진입 시 연결하고 현재 보고 있는 방 하나만 구독하며, 미구독 중 메시지는 재진입 시 HTTP 이력으로 조회한다. 활성 참여자가 다시 입장하면 가입 전과 퇴장 기간을 포함한 전체 공개 이력을 볼 수 있다. 채팅방 목록은 가입 사용자 수 `memberCount`와 현재 실시간 입장 사용자 수 `activeUserCount`를 각각 제공한다. 채팅 시각은 UTC로 저장·전달하고 클라이언트가 현지 시간대로 표시한다. 메시지는 `createdAt`, `messageId` 순으로 정렬하며 텍스트는 정규화 후 Unicode 코드 포인트 기준 1~1,000자로 제한하고 최소 300ms·10초당 10개·동일 텍스트 30초 제한을 적용한다. 메시지는 발신 사용자 ID만 저장하고 이력과 이벤트에는 조회·발행 시점의 현재 닉네임과 프로필 이미지를 반환한다. 탈퇴 사용자는 `userName="탈퇴한 사용자"`만 반환하고 `publicId`와 `profileImageUrl`은 생략한다.

새 채팅 정책 버전이 활성화되면 다시 동의해야 한다. 정책은 관리자 API 없이 운영 마이그레이션으로 추가하고 기존 정책 폐기와 새 정책 활성화를 한 트랜잭션으로 처리하며 활성 정책 본문은 직접 수정하지 않는다. 기존 참여자는 연결·구독·읽기를 유지하지만 다시 동의할 때까지 전송만 차단하며 현재 모든 세션에 정책 동의 필요 이벤트를 전달한다. 채팅방 퇴장 시 사용자의 모든 세션에서 해당 방 구독을 무효화하고 `activeUserCount`에서 제거하되 WebSocket 연결은 유지한다. 재입장 시 기존 가입 행의 `joined_at`을 갱신하고 `left_at`을 비우며 최초 가입 시각은 별도로 저장하지 않는다. 정책 위반 메시지는 `BLOCKED`로 저장하지만 다른 사용자에게 전송하거나 일반 이력에 노출하지 않는다. 위반 누적은 과속·동일 텍스트 반복·금지 표현에만 적용하며 인증·인가·payload·이미지·서버 오류는 포함하지 않는다. 금지 표현은 RDB `chat_prohibited_terms`를 기준정보로 사용하고 서버 시작 시 로컬 메모리에 적재한 뒤 60초마다 불변 목록을 교체한다. 관리자 API 없이 운영 마이그레이션으로 행을 추가·비활성화하고 적발 금칙어 FK나 문자열 스냅샷은 저장하지 않는다. 갱신 실패 시 기존 목록을 유지하고 최초 적재 실패 시 메시지 전송 기능을 비활성화하며 Redis는 사용하지 않는다. 위반 3회마다 제재를 적용하고 다음 제재 판단용 위반 횟수는 다시 0부터 계산하되 위반·제재 이력과 제재 단계는 유지한다. 제재는 사용자 전체의 메시지 전송만 막고 채팅방 입장·구독·실시간 수신과 과거 이력 조회는 허용한다. 제재 만료는 DB UTC 시각 조건으로 판정하고 조회·전송 시 `EXPIRED`로 갱신하며 수동 해제는 제공하지 않는다. 동일 사용자의 여러 세션에서 발생한 동시 전송은 별도 사용자 단위 직렬화 없이 독립 처리하고 사용자별 `clientMessageId` UNIQUE 제약으로 중복 저장만 방지한다. 일반 공개 메시지와 위반·제재 근거 메시지는 기간 만료 삭제 없이 보관하고 메시지 삭제·신고·사용자 차단과 별도 STOMP frame 크기 제한은 초기 범위에 포함하지 않는다. `sub_regions`에 등록된 모든 하위 지역에 채팅방을 미리 생성하며 사용자용 생성·수정·삭제 API를 제공하지 않는다. 이미지 업로드 준비는 멱등성 Key를 사용하지 않아 재요청마다 새 `uploadKey`를 발급하고, 각 Key는 이미지 메시지 Commit 성공 후 재사용할 수 없다.
...
| V1 AI 장소 추천 호출 | 별도 AI 연동 컴포넌트 | 동기 일정 생성 흐름에서 장소 6개를 동선 계산 컴포넌트에 전달 |
...
- 동일 사용자 다중 세션의 실제 동시 전송량과 위반·제재 정합성 문제가 확인될 때 적용할 사용자별 Lock·Queue 또는 DB 동시성 제어 방식
...
- 채팅방 가입 상태는 RDB에 유지하고 현재 실시간 입장 상태는 STOMP 구독 기준으로 인스턴스 메모리에서 관리한다. 목록은 두 상태를 `memberCount`와 `activeUserCount`로 구분해 제공한다.
