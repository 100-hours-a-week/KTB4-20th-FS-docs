...
CREATE TABLE `chat_policy_versions` (
	...
	`active_slot`	TINYINT	GENERATED ALWAYS AS (CASE WHEN `status` = 'ACTIVE' THEN 1 ELSE NULL END) STORED	COMMENT 'ACTIVE 정책 하나만 허용하는 UNIQUE 슬롯',
...
ALTER TABLE `chat_policy_versions` ADD CONSTRAINT `UK_CHAT_POLICY_VERSIONS_ACTIVE_SLOT` UNIQUE (
	`active_slot`
);
